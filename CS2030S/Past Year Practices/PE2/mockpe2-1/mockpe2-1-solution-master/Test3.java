...
class Test3 {
  public static void main(String[] args) {
    ...
    i.expectReturn("Trace.of(10).map(x -> f(x)).map(x -> g(x)).get().equals(Trace.of(10).map(x -> g(f(x))).get())",
        () -> {
    ...
    i.expectReturn("Trace.of(10).map(x -> f(x)).map(x -> g(x)).equals(Trace.of(10).map(x -> g(f(x))))",
        () -> {
          Transformer<Integer,Integer> f = x -> x + 1;
          Transformer<Integer,Integer> g = x -> x * 10;
          Transformer<Integer,Integer> h = x -> g.transform(f.transform(x));
