...
public class ShiftRegisterTest {
    ...
    /**
     * Tests setSeed with simple example.
     */
    @Test
    public void testSetSeed() {
        ...
        r.setSeed(seed);
        int[] expected = {0,1,0,1,1,1,1,0,1};

        for (int i=0; i<9; i++) {
            assertEquals(expected[i], r.getSeed()[i]);
        }
    }

    /**
     * Tests setSeed with invalid example.
     */
    @Test
    public void testSetInvalidSeed() {
        ShiftRegister r = new ShiftRegister(9, 7);
        int[] seed = {0,1,0,1,1,1,1,0,4};
        r.setSeed(seed);

        assertEquals(null, r.getSeed());
    }
    ...
    public void testError() {
        ...
        // In this case, where the seed is of length greater than the size, an error should be thrown to
        // alert the caller that the length of the seed should be equal to size.
        // We can test such a case by asserting that an error is thrown by the function setSeed
        r.setSeed(seed);
        r.shift();
        r.generate(4);
    }
