...
public class GameTree {
    ...
    /**
     * findValue determines the value for every node in the game tree and sets the value field of each node.
     * If the root is null (i.e., no tree exists), then it returns Integer.MIN_VALUE.
     *
     * @return value of the root node of the game tree
     */
    int findValue() {
        return minMax(this.root, true);
    }

    public int minMax(TreeNode root, boolean maxTurn) {
        if (root.leaf) {
            return root.value;
        }
        ...
        if (maxTurn) {
            ...
            for (TreeNode child : root.children) {
                child.value = minMax(child, !maxTurn);
    ...
    public static void play(Game game) {
        ...
        while (!node.leaf) {
            tree.drawBoard(node.name);

            System.out.println("YOU Are X. Where would you like to place it?");
            int pos = Character.getNumericValue(scanner.nextLine().charAt(0));

            while (node.name.charAt(pos) != EMPTYCHAR) {
                // cannot place X here
                System.out.println("Invalid position. Please try again");
                pos = Character.getNumericValue(scanner.nextLine().charAt(0));
            }
            // search children for the correct game board
            for (TreeNode child : node.children) {
                if (child.name.charAt(pos) != EMPTYCHAR) {
                    // found the correct game board
                    // try to minimise
                    if (child.leaf) {
                        node = child;
                        break;
                    ...
                    System.out.println("\nYOU played X at position " + pos);
                    tree.drawBoard(child.name);

                    TreeNode optimalMove = new TreeNode();
                    optimalMove.value = Integer.MAX_VALUE;

                    for (TreeNode grandchild : child.children) {
                        if (grandchild.value < optimalMove.value) {
                            optimalMove = grandchild;
                        }
                    }
                    // play the move
                    node = optimalMove;
                    System.out.println("\nCOMPUTER played the optimal move");
                    break;
                }
            }
        }
